...
describe('#helpers math', function() {
  context('when using minmax', function() {
    it('should return an object with min and max values', function() {
      ...
      expect(minmax(['Abc', 'abc', 'a'])).to.be.an('object').and.to.have.property('min', 'a');
      expect(minmax(['ékn', 'ekn'])).to.be.an('object').and.to.have.property('max', 'ékn');
    });
